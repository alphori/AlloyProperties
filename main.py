...
while True:
    try:
        ...
        if user_concentration not in range(0, 101):
        ...
        if user_concentration > max_concentration +10:
            print(f'Указанное значение концентрации лежит далеко от интервала концентраций (0-{max_concentration}%), '
...
hardness_model = np.poly1d(np.polyfit(concentration, hardness, 2))
toughness_model = np.poly1d(np.polyfit(concentration, toughness, 2))
plasticity_model = np.poly1d(np.polyfit(concentration, plasticity, 2))
...
print(f'Механические свойства сплава '
      f'{alloy_dict[user_system][0][:2]}-{user_concentration}%{alloy_dict[user_system][0][3:-1]}\n'
      f'\n'
